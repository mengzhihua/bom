...
public final class AccessPolicy {
    ...
    public static boolean allows(String role, String method, String path) {
        ...
        if (User.PLANNER.equals(role)) {
            return path.startsWith("/api/boms") || path.startsWith("/api/process")
                    || path.startsWith("/api/ecns") || path.startsWith("/api/integration");
        }
        return false;
    }

    private static boolean engineerWrite(String path) {
        return path.startsWith("/api/parts") || path.startsWith("/api/boms")
                || path.startsWith("/api/ecrs") || path.startsWith("/api/ecns")
