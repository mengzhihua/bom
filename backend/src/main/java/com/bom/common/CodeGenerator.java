...
public class CodeGenerator {
    ...
    public String next(String prefix) {
        ...
        if (updated == 0) {
            try {
                jdbc.update("INSERT INTO bom_sequence (prefix, day_key, seq_value) VALUES (?, ?, 1)", prefix, day);
            } catch (DuplicateKeyException e) {
                jdbc.update("UPDATE bom_sequence SET seq_value = seq_value + 1 WHERE prefix = ? AND day_key = ?", prefix, day);
            }
        }
        Integer n = jdbc.queryForObject("SELECT seq_value FROM bom_sequence WHERE prefix = ? AND day_key = ?", Integer.class, prefix, day);
